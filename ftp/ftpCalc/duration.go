...
func calcDurationFlowStart(acct acctInfo, curve []curveInfo, ct map[string]curveStruct, log calcLog.Calclog) string {
	//new instance for every cashflow calcuate
	r := new(durationCashFlow)
	...
	st := r.durationCashFlow_getTOfFirstDate(start_date, 0)
	...
	case "800":
		return r.durationCashFlow_800(st, start_date, end_date)
	case "700":
		return r.durationCashFlow_700(st, start_date, end_date)
	case "600":
		term, mult := r.getTerm()
		return linearInterpolation(start_date, term, mult, r.curve, r.ct)
	case "500":
		return r.durationCashFlow_500(st, start_date, end_date)
	}
	return ""
}

func (this *durationCashFlow) getTerm() (string, string) {
	term := ""
	mult := ""
...
func (this *durationCashFlow) getDate() string {
...
func (this *durationCashFlow) getEndDate() string {
	rateType := this.acct.adjustable_type_cd
	date := ""
	switch rateType {
	case "0":
...
// durationCashFlow_getTOfFirstDate function:
...
func (this *durationCashFlow) durationCashFlow_getTOfFirstDate(last_reprice_date string, sflag int) string {
...
func (this *durationCashFlow) getACCDays(date string, sflag int) (int, string) {
	rst := 0
	next_date := ""
	//将支付频率换成数字行
	num, _ := strconv.Atoi(this.acct.pmt_freq)
...
func (this *durationCashFlow) durationCashFlow_800(t_first string, last_reprice_date string, next_reprice_date string) string {

	var one durationCash
	var rst []durationCash
	//还款日当年总天数
	var ACC_TOTAL = 0
	//本次还款，计息累计天数
	var ACC_PMT_TOTAL = 0
	...
	for i := 1; utils.CompareDate(NEXT_DATE, next_reprice_date) > 0; i++ {
		...
		this.writeMsg(this.acct.account_number, INDEX, fc, CUR_NET_RATE,
			ACC_PMT_TOTAL, ACC_TOTAL, DISCOUNTING_DAYS, DISCOUNTING_TOTAL_YEAR, CUR_DATE, pv, "")
	...
	if utils.CompareDate(NEXT_DATE, next_reprice_date) <= 0 {
		...
		ACC_PMT_TOTAL, _ = this.getACCDays(next_reprice_date, 0)
		...
		if next_reprice_date == this.acct.maturity_date {
			...
			this.writeMsg(this.acct.account_number, INDEX+1, fc, CUR_NET_RATE,
				ACC_PMT_TOTAL, ACC_TOTAL, DISCOUNTING_DAYS, DISCOUNTING_TOTAL_YEAR, next_reprice_date, pv, "")
			...
			this.writeMsg(this.acct.account_number, INDEX+1, fc, CUR_NET_RATE,
				0, 0, DISCOUNTING_DAYS, DISCOUNTING_TOTAL_YEAR, next_reprice_date, pv, "")
	...
	return strconv.FormatFloat(pvtr/pvt, 'f', 0, 32)
}

func (this *durationCashFlow) durationCashFlow_700(t_first string, last_reprice_date string, next_reprice_date string) string {
...
func (this *durationCashFlow) durationCashFlow_500(t_first string, last_reprice_date string, next_reprice_date string) string {
...
func (this *durationCashFlow) getTotalDaysOfYear(date string) int {
...
func (this *durationCashFlow) getTotalDaysOfAccrual(id string, date string) int {
	t := 0
	switch id {
	case "1", "2":
		t = 360
...
func (this *durationCashFlow) writeMsg(v1 string,
