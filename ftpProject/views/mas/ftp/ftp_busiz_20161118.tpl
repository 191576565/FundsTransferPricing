...
<html>
  ...
  box-shadow: 0 4px 40px #70ac76;*/
			...
			.cd-accordion-menu a::after {
				/* image icon */
				left: 36px;
				/*background-image: url('/static/img/cd-icons.svg') no-repeat -48px 0;*/
						...
						<div class="ibox float-e-margins full-height">
							...
							<div class="ibox-content" style="overflow-x: auto;height: 95%;">
								<div class="row full-height">
									<div class="col-xs-5 full-height" style="background-color: #eee;padding: 0px;overflow: auto;">
										<!--<div class="ibox float-e-margins full-height">
							<div class="ibox-title">
								<div class="row">
									<div class="col-xs-4">
	...
	<script type="text/javascript">
		...
		function addHoverDom(treeId, treeNode) {
			...
			var addStr = "<span class='addBtn res' id='addBtn-" + treeNode.tId +
				"' title='add node' onfocus='this.blur();' resid='208010201000'>+</span>";
			sObj.after(addStr);
		...
		$(function() {
			...
			$.get('/platform/DefaultMenu', {
				...
				if(rs != null) {
					rs.forEach(function(e) {
						if(e.Res_id === '208010201000') { //新增非button按钮
							addBtn = true;
						} else {
			...
			$('.cd-accordion-menu').parent().height($('.wrapper').height() - 100);

			$.get('/platform/FtpBusiz?r=' + Math.random(100000000000000), function(data) {
				$('.cd-accordion-menu').html('');
				var rs = JSON.parse(data);
				lists = rs;
				if(rs != null) {
					rs.forEach(function(e, index) {
						...
						if(e.type === '0') { //节点
							if(e.pId === '-1') {
								temp = '<li class="has-children"><input type="checkbox" name ="' + e.id + '" id="' + e.id + '" checked><label onmouseenter="enter(this)"   onmouseleave="leave(this)" for="' + e.id + '">' + e.name + '</label><ul index="' + index + '" class="' + e.id + '"></ul></li>';
								$('.cd-accordion-menu').append(temp);
							} else {
								temp = '<li class="has-children"><input type="checkbox" name ="' + e.id + '" id="' + e.id + '"><label onmouseenter="enter(this)"   onmouseleave="leave(this)" onclick="goToEdit(' + index + ',this)" for="' + e.id + '">' + e.name + '</label><ul index="' + index + '" class="' + e.id + '"></ul></li>';
								$('.' + e.pId).append(temp);
							}
						} else { //叶子
							temp = '<li><a onclick="goToEdit(' + index + ', this)" href="#">' + e.name + '</a></li>'
							$('.' + e.pId).append(temp);
						}
					});
				}
		...
		function enter(obj){
			$(obj).html($(obj).html()+"<span onclick='addBusiz(this)'>&nbsp;+</span>");
		}
...
//			$('#Ftp_flag').change();
//			$('#ftp_method_id').change();
			var l=lists[$(obj).parent().next().attr('index')];
			$('#Busiz_up_name').val(l.name.split(' ')[1]);
			$('#Busiz_up_id').val(l.id);
			$('#Domain_id_B').val(l.region);
			//$('#Domain_id_B').attr('disabled','disabled');
		...
		function goToEdit(index, obj) {
			var l = lists[index];
			$('#bCode').val(l.id);
			$('#bName').val(l.name.split(' ')[1]);
			if(!l.pId) {
				$('#bPCode').val('-1');
				$('#bPName').val('');
				$('#bPName').attr('disabled', true);
				...
				var pIndex = $(obj).parent().parent().attr('index');
				var ll = lists[pIndex];
				//				var parentNode = treeObj.getNodeByTId(treeNode.parentTId);
				$('#bPCode').val(ll.id);
				$('#bPName').val(ll.name.split(' ')[1]);
				$('#bPName').attr('disabled', false);
				$('#PBtn').attr('disabled', false);
			}
			$('#bType').val(l.type);
			$('#bPrice').val(l.isPrice).change();
			$('#bRegion').val(l.region);

			$('#bMethod').val(l.Ftp_method_id).change();
			$('#bCurve').val(l.Curve_id);
			$('#bTerm').val(l.Term_cd);
			$('#bTermUnit').val(l.Term_cd_mult);
			$('#bSpot').val(l.Point_val);
			...
			$.get('/platform/FtpAdjustment' + '?r=' + Math.random(100000000000000), function(data) {
				...
				rs.forEach(function(obj) {
					var v = obj.AdjustmentId + " " + obj.AdjtypeId;
					if(l.Adjment_info.indexOf(obj.AdjustmentId) >= 0) {
						$("#innerAdjust").append("<option selected value='" + v + "'>" + obj.AdjustmentName + "</option>");
					} else {
						$("#innerAdjust").append("<option value='" + v + "'>" + obj.AdjustmentName + "</option>");
					}
		...
		function delBusiz() {
			...
			swal({
				...
				$.ajax({
					type: "delete",
					url: "/platform/FtpBusiz?Busiz_id=" + $("#bCode").val() + "&Domain_id=" + $('#bRegion').val() + '&r=' + Math.random(100000000000000),
					error: function(msg) {
						console.log(msg.responseText);
					},
					success: function(data) {
