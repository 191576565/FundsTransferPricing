...
<html>
		...
		<div id="wrapper" style="height: 100%;">
			...
			<div class="wrapper wrapper-content animated fadeInRight full-height">
				<div class="row">
					...
					<div class="col-xs-7" style="position: fixed;left:42%;top:20px;">
								...
								<form role="form" id="bInfo">
									...
									<div class="row" id="priceInfo_B">
										<div class="col-xs-12">
											<div class="form-group">
												<label>指定曲线:</label>
												<select name="curve_id" id="bCurve" multiple="multiple" class="form-control" style="border: 1px solid #e5e6e7;">
												</select>
											</div>
										</div>
										<div class="col-xs-4">
											<div class="form-group">
												<label>期限:</label>
												<input type="" name="term_cd" id="bTerm" class="form-control">
											</div>
										</div>
										<div class="col-xs-4">
											<div class="form-group">
								...
								<form role="form" id="a_bInfo">
									...
									<div class="row" id="priceInfo">
										<div class="col-xs-12">
											<div class="form-group">
												<label>指定曲线:</label>
												<select name="curve_id" id="curve_id" multiple="multiple" class="form-control">
												</select>
											</div>
										</div>
										<div class="col-xs-4">
	...
	<script type="text/javascript">
		...
		function zTreeOnDblClickSimple(event, treeId, treeNode) {
			...
			if($('#bCode').val()===treeNode.id || $('#Busiz_id').val()===treeNode.id){
					toastr.warning('业务单元编码与上级业务单元编码冲突');
					return;
				}
			if(type === 'add') {
...
//			#('#wrapper').height(document.body.clientHeight);
			//控制按钮权限
			$.get('/platform/DefaultMenu',{TypeId:2,Id:'208010200000'}, function(data){
				var rs=JSON.parse(data);
				//将所有有权限编码的按钮先禁用
...
//							$("button[resid='"+e.Res_id+"']").attr('disabled', false);
			...
			$.get('/platform/FtpAdjustment', function(data) {
...
//				appendOption('inner', data, 'AdjustmentId', 'AdjustmentName');
		...
		function inputOnchange(obj, type) {
			...
			if(type === '1') {
				if(v === '0') { //定价
					$('#priceInfo').slideDown();
					$('#ftp_method_id').parent().parent().show();
					$('#ftp_method_id').change();
					showAdjust();
				} else { //不定价
					$('#priceInfo').slideUp();
					$('#ftp_method_id').parent().parent().hide();
				}
			} else if(type === '2') {
				if(v === '0') { //定价
					$('#priceInfo_B').slideDown();
					$('#bMethod').parent().parent().show();
					$('#bMethod').change();
					showAdjust();
				} else { //不定价
					$('#priceInfo_B').slideUp();
					$('#bMethod').parent().parent().hide();
				}
			} else if(type==='3'){
				var infos=[];
//				$(obj).parent().parent().nextAll().hide();
				$('#priceInfo').children().hide();
