...
<html>
						...
						<div class="ibox float-e-margins" style="border: 1px solid #dcdada;">
							<div class="ibox-title" style="border: 0px;">
								<div class="ibox-tools">
									<h3 style="float: left;margin-top: 0px;" id="curveTitle">xxx曲线点值</h3>
							...
							<div class="ibox-content">
								<div class="row">
									<div class="col-xs-4">
										<button resid="30208010104010" class="btn btn-primary res " type="button" onclick="showCVSDialog('add')"> <i class="fa fa-plus"></i> 新增</button>
										<button resid="30208010104020" class="btn btn-info editBtn res" type="button" onclick="showCVSDialog('edit')"><i class="fa fa-paste"></i> 编辑</button>
										<button resid="30208010104030" class="btn btn-danger res" type="button" onclick="deleteCVS()"><i class="fa fa-times"></i> 删除</button>
									</div>
					...
					<div class="col-xs-12" id="curveInfos">
						<div class="ibox float-e-margins">
							<div class="ibox-title">
								<div class="row">
									<div class="col-xs-6">
										<button resid="30208010101000" class="btn btn-primary res" type="button" onclick="showEditCuverDialog('add')"> <i class="fa fa-plus"></i> 新增</button>
										<button resid="30208010102000" class="btn btn-info editBtn res" type="button" onclick="showEditCuverDialog('edit')"><i class="fa fa-paste"></i> 编辑</button>
										<button resid="30208010103000" class="btn btn-danger deleteBtn res" type="button"><i class="fa fa-times"></i> 删除</button>
										<button resid="30208010104000" class="btn btn-warning infoBtn res" type="button" onclick="showCurveValueData()"><i class="fa fa-flag"></i> 查看曲线点值</button>
									</div>
							...
							<div class="col-sm-12 b-r">
								<form role="form" id="curveForm">
									<div class="form-group col-sm-6">
										<label>曲线编号:</label>
										<input type="text" placeholder="" class="form-control" name="Curve_id" id="curveCode">
									</div>
									<div class="form-group col-sm-6">
										<label>曲线名称:</label>
	...
	<script type="text/javascript">
		...
		$(function() {
			//控制按钮权限
			$.get('/platform/DefaultMenu', { TypeId: 2, Id: '30208010100000', r: Math.random() * 10000000000000 }, function(data) {
			...
			$.get('/platform/FtpRepType', { r: Math.random() * 10000000000000 }, function(data) {
				...
				if(rs != null) {
					rs.forEach(function(obj) {
						$("#repeat").append("<option flag='" + obj.FtpRepAttr + "' value='" + obj.FtpRepId + "'>" + obj.FtpRepDesc + "</option>");
					});
				}
			});
		...
		function saveCurveInfo() {
			...
			var rep = $('#repeat').val();
			var repv = $('#repeat option:selected').attr('flag');
			var terms = '';
		...
		function showCVSDialog(type) {
			...
			if(type === 'add') {
				...
				if($('.cvs :checked').length > 1) {
					toastr.warning('请只选择一行曲线点值信息进行编辑');
					return;
				}
		...
		function saveCVSInfo() { //需求修改点值不能为空
			...
			if($('#opType').val() == "add") {
				...
				$.ajax({
					type: "put",
					url: "/platform/FtpCurveInfoStruct?" + '&Curve_Id=' + Curve_Id + '&Domain_Id=' + Domain_Id + '&date=' + day + '&JSON=' + JSON.stringify(spots) + '&r=' + Math.random(100000000000000),
					error: function(msg) {
						console.log(msg.responseText);
					},
					success: function(data) {
		...
		function inputOnchange(obj) {
			...
			if(type === '1') {
				$('#repeat').parent().show();
				if($('#repeat').val() == null) {
					$('#repeat option:first').attr('selected', true);
				}
			} else {
				$('#repeat').parent().hide();
			}
