			...
			$.get('/platform/BusizInfoCalc', {r:Math.random()*10000000000000}, function(data) {
				...
				if(rs!=null){
					rs.forEach(function(obj) {
						$("#BusinessId").append("<option alflag='"+obj.Al_flag+"' method='" + obj.Method_id + "' value='" + obj.Busiz_id + "'>" + obj.Busiz_desc + "</option>");
					});
				}
			});
...
//			var busizId = $('#BusinessId').find("option:selected").attr('value');
			var alflag = $('#BusinessId').find("option:selected").attr('alflag');
				...
				if(rs.ErrorCode === '1') {
					...
					if(rs.Insideinfo != null) {
						for(var i = 0; i < rs.Insideinfo.length; i++) {
							if(rs.Insideinfo[i].Adjustname == "司库利润还原调节"){
