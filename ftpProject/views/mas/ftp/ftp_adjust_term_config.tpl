...
//						$("button[resid='"+e.Res_id+"']").attr('disabled', false);
			...
			$.get('/mas/ftp/adjust/termLiq/tlp', {CurveType:1, r:Math.random()*10000000000000}, function(data){
				var rs=JSON.parse(data);
				if(rs!=null){
					$('.tlps tbody').html('');
		...
		function initTable() {
			currs = $('.currs').DataTable({
				sAjaxSource: '/mas/ftp/adjust/termLiq/config?AdjId=601&r='+Math.random()*10000000000000,
				info: true,
				iDisplayLength: 15,
				aoColumns: [{
					"data": null
...
//			}
			...
			if(type===1){
				$.post('/mas/ftp/adjust/termLiq/config?JSON='+JSON.stringify(json)+"&BusizId="+Busiz_id+"&r="+Math.random()*10000000000000,function(data){
					var rs=JSON.parse(data);
		            if(rs.ErrorCode==='1'){
		                toastr.success('调节项信息保存成功！');
//						currs.ajax.reload(null, true);
						currs.ajax.url('/mas/ftp/adjust/termLiq/config?AdjId=601&r='+Math.random()*10000000000000).load();
				...
				$.ajax({
					type:"put",
					url:'/mas/ftp/adjust/termLiq/config?JSON='+JSON.stringify(json)+"&BusizId="+Busiz_id+"&r="+Math.random()*10000000000000,
					async:true,
					error: function(msg){
