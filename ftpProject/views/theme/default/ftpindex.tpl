		<nav id="wrapper" class="navbar-static-side theme-bg-color"  style="margin-left: 0px;width: 1000px;">
			<div class="H-logo-area" style="font-size: 30px;width: 100%;padding-left: 20px;text-align: left;margin-bottom: 20px;margin-top: 5px;">
		...
		<div id="page-wrapper" class="container-fluid gray-bg" style="margin: 0px 0px;">
			...
			<div class="row H-content-tab" style=""> 
...
//	         </div>
			...
			if(rs!=null){
				...
				rs.forEach(function(e){
					if(e.Res_up_id==='30208000000000'){
						var temp="<div class='col-sm-4'><div class='tile-group double' style='margin-left:40px;'><span class='tile-group-title'>"+e.Res_name+"</span><div class='tile-container' style='margin-top:5px;' resid='"+e.Res_id+"'></div></div></div>"
		...
		$.get('/platform/MenuPage',{TypeId:1,Id:'208000000000',r:Math.random()*100000000000},succ);
	})
	...
	var platMenuService = function(e){
		if($(e).attr('data-id')==='30208050200000'){
			$.get('/platform/MenuPage', function(){
				//取消勾选
//				$('input:checkbox').parent().iCheck('uncheck');
				
				$('#exportDialog').modal('show');
			});
			return ;
		}else if($(e).attr('data-id')==='30208050100000'){
			$.get('/platform/MenuPage', function(){
			    swal({
	                title: "警告",
			        text: "全量导入会覆盖之前的数据，请谨慎导入！",
	                type: "warning"
	            });
				
				$('#uploadFormAll')[0].reset();
				$('#uploadFormCurve')[0].reset();
				$('#uploadFormPolicy')[0].reset();
	...
	//					var cot = '<div data-type='frame' data-id=''+data_id+'" class='active'>'+data+'</div>';
			var cot = "<div data-type='frame' data-id='"+data_id+"' class='active' style='width:100%;height:100%;padding:0px 0px;overflow: hidden;'>"
						+"<iframe src='"+url+"' width='100%' height='100%' class='ff'></iframe>"
						+"</div>";
