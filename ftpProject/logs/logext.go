...
var Std = New(os.Stderr, "[FTP] ", Ldefault())
...
func init() {
	var err error
	logpath := conf.FtpConf.FtpLog
	w := NewFileWriter(FileOptions{
		ByType:   ByDay,
		FileName: "FTP",
		Dir:      logpath})

	w1 := io.MultiWriter(w, os.Stdout)
	SetOutput(w1)

	lvl := conf.FtpConf.LogLvl
	lvlint := 0
	if lvl == "" {
		lvlint = 0
	} else {
	...
	SetOutputLevel(lvlint) //Ldebug)
	Std.Info("日志初始化成功，级别为：", lvlint)
	//Log.SetOutput(w)
	//Log.SetOutputLevel(lvlint)
}
